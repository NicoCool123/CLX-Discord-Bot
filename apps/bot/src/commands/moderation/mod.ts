...
export default {
  data: new SlashCommandBuilder()
    ...
    // /mod reason <casenumber> <reason>
    .addSubcommand((sub) =>
      sub
        .setName('reason')
        .setDescription('Edit the reason for an existing case')
        .addIntegerOption((o) =>
          o.setName('casenumber').setDescription('Case number').setRequired(true).setMinValue(1),
        )
        .addStringOption((o) =>
  ...
  async execute(interaction) {
    ...
    if (sub === 'history') {
      ...
      if (records.length === 0) {
        ...
        const lines = records.map((inf) => {
          ...
          return `${emoji} \`#${inf.caseNumber}\` **${inf.type}**${dur} — ${inf.reason.slice(0, 70)}\n  <t:${ts}:R> · <@${inf.moderatorId}>`;
        });
    ...
    if (sub === 'reason') {
      await interaction.deferReply({ flags: MessageFlags.Ephemeral });
      const caseNumber = interaction.options.getInteger('casenumber', true);
      ...
      const embed = new EmbedBuilder()
        ...
        .setDescription(`✅ Updated reason for case \`#${caseNumber}\` (**${infraction.type}**).\n> ${reason}`);
