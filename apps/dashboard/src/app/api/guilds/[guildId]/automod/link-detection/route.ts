...
export async function POST(
  ...
  await Promise.all([
    db.automodRule.upsert({
      where: { guildId_type: { guildId, type: AutomodRuleType.LINK_DETECTION } },
      create: { guildId, type: AutomodRuleType.LINK_DETECTION, action, enabled },
      update: { action, enabled },
    }),
    db.guildSettings.upsert({
      where: { guildId },
      create: { guildId, allowedLinks },
      update: { allowedLinks },
    }),
  ]);
